...
const retrieveAssessment = async function retrieveAssessment (platform) {
  ...
  const response = await rest.GETRequestWrapper(retrieveAssessment.name, config.NOWSECURE.URI, config.NOWSECURE.ACCESS_TOKEN, nowSecureEndpoint);
...
const retrieveAssessmentResults = async function retrieveAssessmentResults (platform, task) {
  ...
  const response = await rest.GETRequestWrapper(retrieveAssessmentResults.name, config.NOWSECURE.URI, config.NOWSECURE.ACCESS_TOKEN, nowSecureEndpoint);
  return response;
};
